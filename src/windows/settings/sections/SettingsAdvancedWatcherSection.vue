<template>
  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.menu.advanced.windowWatcher') }}</h2>
    ...
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.description') }}</p>
    <div class="settings-section__actions">
      <n-button :disabled="!dirty || saving" @click="resetDraft">{{ $t('settings.advanced.watcher.discardChanges') }}</n-button>
      <n-button type="primary" :loading="saving" :disabled="!canSave" @click="saveDraft">{{ $t('settings.advanced.watcher.saveChanges') }}</n-button>
      <n-button tertiary type="error" :loading="saving" @click="resetPersisted">{{ $t('settings.advanced.watcher.resetDefault') }}</n-button>
    </div>
  </section>

  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.advanced.watcher.basicSwitches') }}</h2>
    </div>
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.basicSwitchesDesc') }}</p>
  ...
  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.advanced.watcher.monitorFrequency') }}</h2>
    </div>
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.monitorFrequencyDesc') }}</p>
  ...
  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.advanced.watcher.monitorEvents') }}</h2>
    </div>
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.monitorEventsDesc') }}</p>
  ...
  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.advanced.watcher.aiResponseMode') }}</h2>
    </div>
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.aiResponseModeDesc') }}</p>

    <n-radio-group v-model:value="draftConfig.aiResponse.mode">
      <n-space direction="vertical">
        <n-radio value="first-open">{{ $t('settings.advanced.watcher.aiModeFirstOpen') }}</n-radio>
        <n-radio value="every-switch">{{ $t('settings.advanced.watcher.aiModeEverySwitch') }}</n-radio>
        <n-radio value="specific-apps">{{ $t('settings.advanced.watcher.aiModeSpecificApps') }}</n-radio>
      </n-space>
    </n-radio-group>
  ...
  <section class="settings-section">
    <div class="settings-section__header">
      <h2>{{ $t('settings.advanced.watcher.ignoreRules') }}</h2>
    </div>
    <p class="settings-section__desc">{{ $t('settings.advanced.watcher.ignoreRulesDesc') }}</p>
    ...
    <n-form label-placement="top">
      <n-form-item :label="$t('settings.advanced.watcher.ignoreProcessNames')">
        ...
        />
        <template #feedback>{{ $t('settings.advanced.watcher.ignoreProcessNamesFeedback') }}</template>
      </n-form-item>
      <n-form-item :label="$t('settings.advanced.watcher.ignoreTitleKeywords')">
        ...
        />
        <template #feedback>{{ $t('settings.advanced.watcher.ignoreTitleKeywordsFeedback') }}</template>
      </n-form-item>
    </n-form>
  </section>
</template>
