...
- 历史消息、表演记录与统计分析
- 录音链路与全局快捷键，支持按住 / 点击两种录音模式
- 托盘、置顶、鼠标穿透（无穿透 / 智能穿透 / 完全穿透）等桌面助手能力
- 主题色自动提取，支持手动覆盖
- 设置窗口分组导航、快捷跳转（`Ctrl/Cmd+K`）与浅色/深色外观；高级「数据管理」可查看本地存储占用并导入导出配置
...
> 首次启动时，应用会提示下载 Live2D Cubism Core 运行时文件（约 200KB），点击确定即可自动下载；下载失败支持重试，也可跳过下载后在设置中手动补装。
...
在「设置 → 高级 → 数据管理」可查看数据库文件、内嵌媒体、模型库与日志等占用概况，并执行缓存清理、日志导出、配置导入导出等维护操作。消息趋势与内容分布见「历史 → 统计概览」。
