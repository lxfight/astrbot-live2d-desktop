...
### Added
- 新增桌面感知基础能力，可归一化当前应用/窗口身份，并将桌面 RPC 观测同步到感知服务。
- 新增模型配置变更事件，配置保存、删除、自动创建后会广播更新，主窗口可热重载最新模型配置。
- 新增表情请求创建工具与模型配置编辑器 composable，提升别名配置维护能力。

### Changed
- 重组设置页设计 tokens、页面操作区与子区块样式，统一 Naive UI 表单控件主题。
- 优化主窗口覆盖层、径向菜单与设置页视觉细节，提高文字对比度与透明界面的可读性。
- 主进程窗口创建流程提前，改善应用启动速度。

### Fixed
- 修复 AstrBot 端下线后重新上线时，桌面端可能无法收到断线事件并卡住重连的问题。
- 修复 Cubism Core 缺失、模型迁移、后台迁移 marker 与自动加载之间的多处竞态。
- 修复 `model:load` 消息可靠性、pending 模型加载状态、启动时模型加载占位与模型配置保存问题。
- 修复设置页迁移阻塞 ready、透明卡片表面和设置控件对齐问题。
...
### Added
- 设置窗口全新布局：左侧分组导航、顶栏快捷菜单、命令面板（`Ctrl/Cmd+K`）与浅色/深色外观切换。
- 高级「数据管理」本地存储概览：数据库体积、内嵌媒体、模型库、日志、Cubism Core、会话与表演统计及媒体类型分布。
- 配置导入/导出回归数据管理维护区，与连接、行为等设置一并备份恢复。
- 音频口型分析（lip sync）与程序化待机动画；模型行为支持按模型配置。
- 从模型贴图源图更可靠地提取主题色。

### Changed
- 设置各子页统一为页面脚手架与子区块结构；历史、关于、模型库等交互与版式调整。
- 设置窗口默认不再强制置顶，可在标题栏 Pin 切换。
- 精简设置内冗余说明文案（长段提示、重复描述等）。

### Fixed
- 修复模型主题色提取相关表现。

### Docs
- 更新 README、使用教程，说明新设置入口、数据管理与历史统计位置。
